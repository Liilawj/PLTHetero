...
DimPlot(PLTCov, reduction = "tsne",
        ...
        split.by = "cond.idents",label = T,pt.size=1)
```
*base: RNA_snn_res.0.7*
```{r unsup-markers calculation}
...
```
```{r unsup-markers annoVis on PLTf3}
...
```
```{r Add cluI (9 in PLT0.7) (cluster8) enrichment results}
...
```
```{r unsup-markers anno-on all ori-clusters res0.7}
...
DimPlot(BigMPLTintegrated,group.by = "PLT.idents",
  ...
  theme(axis.line = element_blank(),axis.title = element_blank())
```
```{r GO/KEGG of unsupmarkers-on clu2 of res0.7}
...
p1
```
```{r GO/KEGG of unsupmarkers-on clu0 of res0.7}
library(clusterProfiler)
library(org.Hs.eg.db)
library(GOplot)
library(stringr)
...
p1
```
```{r GO/KEGG of unsupmarkers-on clu1 of res0.7}
library(clusterProfiler)
library(org.Hs.eg.db)
library(GOplot)
library(stringr)
...
enrich2plot(clu1.go_Ssiggenes@result) +
  ...
  theme_bw()
```
```{r GO/KEGG of unsupmarkers-on clu3 of res0.7}
library(clusterProfiler)
library(org.Hs.eg.db)
library(GOplot)
library(stringr)
...
enrich2plot(clu3.go_Ssiggenes@result) +
  ...
  theme_bw()
```
```{r GO/KEGG of unsupmarkers-on clu4 of res0.7}
library(clusterProfiler)
library(org.Hs.eg.db)
library(GOplot)
library(stringr)
...
clu4.go_Ssiggenes_lowP
```
```{r GO/KEGG of unsupmarkers-on clu8 of res0.7}
library(clusterProfiler)
library(org.Hs.eg.db)
library(GOplot)
library(stringr)
...
clu8.go_Ssiggenes_lowP
```
```{r GO/KEGG of unsupmarkers-on clu7 of res0.7}
library(clusterProfiler)
library(org.Hs.eg.db)
library(GOplot)
library(stringr)
...
clu7.go_Ssiggenes_lowP
```
```{r GO/KEGG of unsupmarkers-on clu5 of res0.7}
library(clusterProfiler)
library(org.Hs.eg.db)
library(GOplot)
library(stringr)
...
clu5.go_Ssiggenes_lowP
```
```{r GO/KEGG of unsupmarkers-on clu6 of res0.7}
library(clusterProfiler)
library(org.Hs.eg.db)
library(GOplot)
library(stringr)
...
clu6.go_Ssiggenes_lowP
```
```{r GO/KEGG of unsupmarkers-on clu9 of res0.7}
library(dplyr)
markers <- read.table("~/BigMPLTres07markers.txt",sep='\t')
all.top100 <- markers %>% group_by(cluster) %>% top_n(n = 100, wt = avg_log2FC) 
top100 <- all.top100[!duplicated(all.top100$gene),]
...
```
```{r not useful-unsup-markers anno-on all ori-clusters res0.7}
...
table(BigMPLTintegrateduse$predicted.celltypePLTres0.7)
```
```{r reunion BigMPLT-07 to BigMerge}
...
set.seed(1)
```
**Decided to do WGCNA only on Cov group first. So prep the PLTCov obj.**
```{r from BigMPLT prep PLTCov Seurat obj as input}
BigMPLTintegrated <- readRDS("~/BigMPLTintegratedProjected.rds")
PLTCovintegrated = 
...
```
```{r Not use-create metacells-v1 across PLT.idents and MSH.idents}
...
```
```{r create metacells-v2 across PLT.idents and ID}
table(PLTCovintegrateduse.forWGCNA$predicted.celltypePLTres0.7)
...
```
```{r NEW FUNCTION ComputeModuleEigengenenew}
...
```
```{r NEW FUNCTION ModuleEigengenesnew}
ModuleEigengenesnew <- 
...
```
```{r hubgenes enrichment-mapping to tsne-no calc only plot}
...
```
```{r hubgenes enrichment-mapping to umap-no calc only plot}
plot_list1 <- ModuleFeaturePlot(
  PLTCovintegrateduse.forWGCNAv2refpca,
  reduction="ref.umap",
  features = "hMEs",
...
wrap_plots(plot_list3,ncol=3)
```
```{r cor between modules-no calc only plot}
ModuleCorrelogram(PLTCovintegrateduse.forWGCNAv2refpca,
                  exclude_grey = T,
                  features ="MEs")
```
```{r modules expressed in clusters by marker-genes-PLT.idents}
...
```
```{r read-in BigMPLT prep Seurat obj input}
BigMPLTintegrated <- readRDS("~/BigMPLTintegratedProjected.rds")
...
```
```{r NO RUN-NEW FUNCTION ComputeModuleEigengenenew}
ComputeModuleEigengenenew <- 
  function (seurat_obj, cur_mod, modules, group.by.vars = NULL, 
  verbose = TRUE, vars.to.regress = NULL, scale.model.use = "linear", 
  pc_dim = 1, assay = NULL, wgcna_name = NULL,...) {
...
```
```{r NO RUN-NEW FUNCTION ModuleEigengenesnew}
ModuleEigengenesnew <- 
function (seurat_obj, group.by.vars = NULL, modules = NULL, vars.to.regress = NULL, scale.model.use = "linear", verbose = TRUE, assay = NULL, pc_dim = 1, wgcna_name = NULL,...) {
    if (is.null(wgcna_name)) {
        wgcna_name <- seurat_obj@misc$active_wgcna
...
```
```{r Plot pheno cor-Method1forCovnonCov}
...
PlotModuleTraitCorrelation(
  ...
  theme(axis.text.x = element_text(size=10),
        ...
        axis.text.y.left = element_text(size=10))
```
```{r Plot pheno cor-Method2forCovnonCov-FIG3BSUP}
...
wrap_plots(plot_list3,ncol=3)
```
```{r cor between modules-no calc only plot-SUP}
ModuleCorrelogram(BigMPLTintegrateduse.forWGCNAv2refpca,
                  exclude_grey = T,
                  features ="MEs")
```
```{r modules expressed in clusters11111-no use}
library(conflicted)
conflict_prefer("select","dplyr")
...
```
```{r not useful NEW FUNCTION RunEnrichrnew}
...
```
```{r DME between aggre/others：Aimed as unsup anno}
BigMPLTintegrateduse.forWGCNAv2refpca <- readRDS("~/BigMPLTintegrateduse.forWGCNAv2refpca.anotherday2.rds")
...
BigMPLTintegrateduse.forWGCNAv2refpca@assays$RNA@data@x[is.na(BigMPLTintegrateduse.forWGCNAv2refpca@assays$RNA@data@x)] <- 0
```
```{r DME between S/nonS inside immPLT：Aimed function identification}
...
```
```{r RUN HERE PlotDMEsLollipop/PlotLollipop function mannually load}
...
```
```{r NO need run: self debug FindDMEs function}
...
DMEs
```
```{r DME heatmap of SnonS on all PLT sub}
...
p
```
```{r DME heatmap of all MSH.idents}
DMEs_all <- FindAllDMEs(
  BigMPLTintegrateduse.forWGCNAv2refpca,
  group.by = 'MSH.idents'
)
...
p
```
*only for notes, no need run, in Linux already done:*
```{r monocle set up}
...
plot_cell_trajectory(mycds, color_by = c("State"),
  ...
  NoLegend()
```
```{r RUNHERE unsup FINAL using 4th mycds-run in 2ndServer}
suppressMessages({
  suppressWarnings({library(Seurat)
library(monocle)})
})
...
plot_cell_trajectory(mycds, color_by = c("cond.idents")) +
  facet_wrap(~RNA_snn_res.1.5, nrow = 5)
```
```{r PLOT newF2LAST CT using 4th mycds-run in 2ndServer}
library(RColorBrewer)
display.brewer.pal(11,"Set3")
mycol = brewer.pal(11,"Set3")
mycol
...
ggplot(Cellratio,aes(disease.idents,Freq,fill=State))+
  ...
  theme(axis.title.x = element_blank(),axis.line.x = element_blank())
```
```{r new sup CT calculation using the PLT.idens DE genes}
suppressMessages({
  suppressWarnings({library(Seurat)
library(monocle)})
})
...
plot_cell_trajectory(mycds, color_by = c("cond.idents")) +
  facet_wrap(~RNA_snn_res.1.5, nrow = 5)
```
```{r new sup CT calculation using the MSH.idens DE genes}
suppressMessages({
  suppressWarnings({library(Seurat)
library(monocle)})
})
...
plot_cell_trajectory(mycds, color_by = c("cond.idents")) +
  facet_wrap(~RNA_snn_res.1.5, nrow = 5)
```
```{r monocle FULL-After pseudotime analysis-BEAM analysis}
...
saveRDS(mycds_filteredLowDisp,"~/noNormmycdsDDRTree_filteredLowDisprerunOrdercell.rds")
...
str(BEAM_res)
saveRDS(BEAM_res,"Mt-Cleared-Outputs/Secondary_preprocess_codes_output/CT/Unsup_Primary/BEAM_res.rds")
...
```
```{r GO of the BEAM bp3-state89-key is look at state9}
...
saveRDS(Hieclu1.go_Ssiggenes,"~/CT/BEAM/enrich/BP3/top100/Hieclu1.go_siggenes.rds")
...
```
**After this, decided to keep doing the top100 version for other BPs.**
```{r GO of the BEAM bp5-state13-key is look at state1}
...
DimPlot(BigMPLTintegrated,group.by = "Pseudotime",label=F)+
  NoLegend()
```
```{r GSEA on state9 wilxauc DE genes}
library(clusterProfiler)
...
```
S vs. M:
```{r monocle FULL-After pseudotime analysis-SpseudoDE}
mycds <- readRDS("~/noNormmycdsDDRTree3.rds")
mycds <- readRDS("~/noNormmycdsDDRTree3rerunOrdercell.rds")
mycds$Pseudotime
mycds$State
...
```
```{r monocle FULL-After pseudotime analysis-MpseudoDE}
mycds <- readRDS("~/noNormmycdsDDRTree3.rds")
mycds$Pseudotime
mycds$State
levels(mycds$State)
...
```
Cov vs. nonCov:
```{r monocle FULL-After pseudotime analysis-CovpseudoDE}
mycds <- readRDS("~/noNormmycdsDDRTree3.rds")
mycds@assayData$exprs
...
barplot(hclu1.kk,showCategory = 4)
```
```{r monocle FULL-After pseudotime analysis-nonCovpseudoDE}
mycds <- readRDS("~/noNormmycdsDDRTree3.rds")
mycds@assayData$exprs
install.packages("remotes")
devtools::install_github("jacobheng/cellwrangler")
...
hclu3.genes
```
```{r GO KEGG of nonCovpseudoDE-OVERWRITE with CovpseudoDE}
install.packages("clusterProfiler")
install.packages("org.Hs.eg.db")
install.packages("GOplot")
install.packages("stringr")
...
barplot(hclu1.kk,showCategory = 4)
```
```{r Not USEFUL monocle FULL-After clusters intepretation by cluster}
...
ordering_genes <- row.names (subset(diff_test_res, qval < 0.01))
```
```{r monocle FULL-After clusters intepretation by WGCNA}
sessionInfo()
...
```
```{r save CT results to Seurat obj}
mycds <- readRDS("~/noNormmycdsDDRTree3.rds")
BigMPLTintegrateduse.forWGCNAv2refpca <- readRDS("~/BigMPLTintegrateduse.forWGCNAv2refpca.anotherday2.rds")
...
```
```{r reunion res1.5allPLT to CovintegratedAnnorerun2SR}
...
```
*Cov group obj:*
```{r obj read-in and cellchat-obj create and save}
BigMergereU <- readRDS("~/BigMergereU20.rds")
table(BigMergereU$predicted.celltypeBasicwithnewreU)
table(BigMergereU$CCCreU)
BigMergereU <- readRDS("~/BigMergereU.rds")
...
pathways.show.ReUnonCovSpecific <- c("CADM","CD226","GRN",
                                     ...
                                     "PARs")
```
```{r Vis on pathway-chord plot-the onoff pathways from previous chunk}
cellchatCov <- readRDS("~/cellchatCov.rds")
cellchatnonCov <- readRDS("~/cellchatnonCov.rds")
groupSizeCov <- as.numeric(table(cellchatCov@idents))
groupSizeCov
...
netVisual_aggregate(reUScellchat, signaling ="MK",
                    ...
                    signaling.name = "CCL in Cov")
```
```{r Vis on pathway-chord plot-the all sig pathways from previous chunk}
pathways.show.ReUCov <- df.net.ReUCov$pathway_name
levels(as.factor(pathways.show.ReUCov))
pathways.show.ReUnonCov <- df.net.ReUnonCov$pathway_name
levels(as.factor(pathways.show.ReUnonCov))
...
netVisual_individual(reUScellchat, signaling = "CXCL", 
                     pairLR.use = LR.show, layout = "circle")
```
```{r specifiy source or target in bubble plot}
...
```
```{r connect up-CD99-AggreActi strong, messy, separately investigate this-for fig 1}
...
DotPlot(aggrePLT,features=c("CD99"),group.by = "cond.idents")+
  ...
  scale_colour_distiller(palette = "Spectral")
```
```{r connect up-APP-B might corresponding to PLT subgroups changed subgroups while MSH}
...
```
```{r connect up-CCL-Mono-decide not use here}
...
```
```{r connect up-CD45-B-decide not use here}
...
cowplot::plot_grid(p1, p2, p3, align = "v",nrow=3)
```
```{r connect up-CXCL might corresponding to PLT subgroups changed subgroups while MSH}
...
```
```{r connect up-CD22-fromB-decide not use here}
...
```
```{r data prep merge groups-using noK version to merge}
cellchatCov <- readRDS("~/cellchatCov.rds")
cellchatnonCov <- readRDS("~/cellchatnonCov.rds")
cellchatCovnoUnk <- subsetCellChatnew(
  cellchatCov,idents.use = c("B","CD14 Mono","CD16 Mono","CD4 T",
...
```
```{r new function netVisual_diffInteractionnew}
...
draw(ht1 + ht2, ht_gap = unit(0.5, "cm"))
```
```{r violin plot check genes related to LR}
BigMPLTintegrated <- readRDS("~/BigMPLTintegratedProjected.rds")
